...
async function main(): Promise<void> {
  ...
  console.log(owners);
  const RUSDContract = new ethers.Contract(RUSDAddress, CIP36__factory.createInterface(), provider) as CIP36;

  const creditLimit = ethers.utils.formatUnits(await RUSDContract.creditLimitOf(multiSigAddress), "mwei");
  const creditBalance = ethers.utils.formatUnits(await RUSDContract.creditBalanceOf(multiSigAddress), "mwei");
